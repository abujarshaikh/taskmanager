...
export default function UserStatsTable({ stats, loading }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-6 flex flex-col h-full">
      <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">User Stats</h2>
        ...
        <>
          <div className="overflow-x-auto flex-1">
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-xs uppercase tracking-wide">
              ...
              <tbody>
                {paginated.map((s) => {
                  const progress = s.total === 0 ? 0 : Math.round(((s.inProgress + s.completed) / s.total) * 100);
                  return (
                    <tr key={s.username} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                      <td className="px-3 py-3 font-medium text-gray-800 dark:text-gray-100">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-full bg-blue-100 dark:bg-blue-950 text-blue-600 dark:text-blue-400 flex items-center justify-center text-xs font-semibold">
                      ...
                      <td className="text-center px-3 py-3">
                        <span className="bg-yellow-100 dark:bg-yellow-950 text-yellow-700 dark:text-yellow-400 px-2 py-1 rounded-full text-xs font-medium">
                          {s.pending}
                        </span>
                      </td>
                      <td className="text-center px-3 py-3">
                        <span className="bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-400 px-2 py-1 rounded-full text-xs font-medium">
                          {s.inProgress}
                        </span>
                      </td>
                      ...
                      <td className="text-center px-3 py-3">
                        <span className="bg-green-100 dark:bg-green-950 text-green-700 dark:text-green-400 px-2 py-1 rounded-full text-xs font-medium">
                          {s.completed}
                        </span>
                      </td>
                      <td className="text-center px-3 py-3 font-semibold text-gray-700 dark:text-gray-300">{s.total}</td>
                    </tr>
                  );
                })}
              </tbody>
